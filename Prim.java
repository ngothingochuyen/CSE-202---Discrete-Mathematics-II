...
class SampleMath2 {
    ...
    private static Vertex findUnvisitedMincostVertex(Vertex[] vertices) {
        Vertex min = null;
        for (Vertex v : vertices) {
            if (!v.visited) {
                if (min == null || min.cost > v.cost) {
                    min = v;
                }
            }
        }
    ...
    //hieenj thuwcj thuaatj toasn prim
    private static void printPrim2(Vertex[] vertices) {
        vertices[0].previous = null;
        vertices[0].cost = 0;
        int countUnvisitedVertices = vertices.length;
        while (countUnvisitedVertices > 0) {
            Vertex min = findUnvisitedMincostVertex(vertices);
            if (min == null) {
                break;
            } else if (min.cost == Integer.MAX_VALUE) {
                //đồ thị không liên thông
            } else {
                min.visited = true;
                for (Edge e : min.adjacentVertices) {
                    Vertex w = e.endpoint;
                    if (!w.visited && w.cost > e.weight) {
                        w.cost = e.weight;
                        w.previous = min;
                    }
                }
        ...
        //cây khung gồm những cạnh nào? thì ...
    }
